...
class Producto
{
    ...
    public function crearProducto()
    {
        if(self::validarProducto($this->nombre, $this->precio, $this->trabajo_id))
        {
            ...
            $consulta = $objAccesoDatos->prepararConsulta("INSERT INTO productos (nombre, precio, trabajo_id, tiempo_estimado) VALUES (:nombre, :precio, :trabajo_id, :tiempo_estimado)");
    ...
    public static function obtenerTodos($rol)
    {
        ...
        if($rol)
        {
            ...
            $condicion = ($rol == 5) ? "" : "WHERE trabajo_id = :rol";
    ...
    {
        $objAccesoDatos = AccesoDatos::obtenerInstancia();
        $consulta = $objAccesoDatos->prepararConsulta("SELECT id_producto, nombre, precio, trabajo_id, tiempo_estimado FROM productos WHERE nombre = :nombre");
    ...
    {
        if($nombre != "" && $precio > 100 && $trabajo_id < 4 && $trabajo_id > 0 && !self::obtenerProducto($nombre))
        {
            return true;
        }
